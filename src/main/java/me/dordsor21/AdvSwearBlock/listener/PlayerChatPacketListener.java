...
public class PlayerChatPacketListener implements Listener {
    ...
    public PlayerChatPacketListener(Main pl, ProtocolManager pM) {
        String[] kw = pl.getConfig().getStringList("ignoring.noIgnoringPacketIfContains").toArray(
            new String[0]);//If the packet contains one of these strings, it cannot be ignored
        ...
        pM.addPacketListener(
            new PacketAdapter(pl, ListenerPriority.HIGHEST, PacketType.Play.Server.CHAT) {
                @Override public void onPacketSending(PacketEvent e) {
                    ...
                    if (!p.hasMetadata("swearBlock") && !(pl.ignoring && pl.ignore
                        .isIgnorer(puuid))) {
                        return;
                    }
                    ...
                    if (wcc == null) {
                        wcc = ComponentConverter.fromBaseComponent(
                            (BaseComponent[]) e.getPacket().getModifier().read(1));
                        isWcc = false;
                    }
                    ...
                    if (!(msg.contains(",{\"color\":\"gold\",\"text\":\"\"}]") || msg
                        ...
                        .contains(",\"hoverEvent\":{\"action\":\"") || msg
                        .equals("{\"text\":\"\"}"))) {
                        ...
                        if (pl.ignoring && pl.ignore.isIgnorer(puuid)) {
                            for (String ignoree : pl.ignore.getIgnored(puuid)) {
                                if (m.toLowerCase().contains(ignoree.toLowerCase()) && Arrays
                                    .stream(kw).noneMatch(m.toLowerCase()::startsWith)) {
                                    e.setCancelled(true);
                                    return;
                                }
                            }
                        ...
                        for (String word : all) {
                            ...
                            while (matcher.find() && matcher.group().length() > word.length()) {
                                ...
                                if (pl.ignoreSwear.stream().anyMatch(Arrays
                                    .asList(matcher.group().toLowerCase().split(" "))::contains)) {
                        ...
                        for (String w : words) {
                            ...
                            if (p.hasMetadata("swearBlock") && !pl.ignoreSwear
                                .contains(temp.toLowerCase()) && Bukkit.getPlayer(temp) == null) {
                                String testTemp =
                                    temp.replaceAll("\\d", "").replace("_", "").toLowerCase();
                                ...
                                if (!(badmul.size() > 1 || badt.size() > 1 || bado.size() > 1 || (
                                    ...
                                    || (badt.size() > 0
                                    && StringUtils.countMatches(testTemp, badt.get(0)) > 1)) || (
                                    bado.size() > 0
                                ...
                                if (multiple || (bad1 != null && !bad1.equals("") && !bad1.isEmpty()
                                    && (testTemp.length() <= multiplier * bad1.length())) || (
                                    bad2 != null && testTemp.length() <= bad2.length() + incr)
                                    || bad3 != null) {
                                    c.append(
                                        w.replaceAll("(((?<!&)[a-fk-o\\d])|[g-jp-zA-Z_])", "*"))
                                        .append(" ");
                                    actuallyEdited = true;
                                    continue;
                                }
                            ...
                            if (Json.stripCodes(w).matches(
                                ...
                                c.append(
                                    ...
                                    .append(http).append(Json.stripCodes(w))
                                    .append("\"},\"text\":\"").append(w)
                                    .append("\"},~~,{\"text\":\" ");
                                continue;
                            }
                            c.append(w).append(" ");
                            ...
                            if (isWcc)
                                ...
                                e.getPacket().getModifier().write(1, ComponentConverter.fromWrapper(
                                    WrappedChatComponent.fromJson(
                                        "[" + chat + ",{\"text\":\"\",\"color\":\"gold\"}]")));
                        } catch (Exception ex) {
                            if (Main.plugin.failSilent) {
                                return;
                            }
                            pl.getLogger()
                                .severe("Error Editing Chat Packet. Please report this to GitLab");
