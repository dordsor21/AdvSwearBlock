...
public class PlayerSignPacketListener implements Listener {
    public PlayerSignPacketListener(Main pl, ProtocolManager pM) {
        ...
        pM.addPacketListener(
            new PacketAdapter(pl, ListenerPriority.HIGHEST, PacketType.Play.Server.MAP_CHUNK,
                ...
                @Override public void onPacketSending(PacketEvent e) {
                    if (e.getPacketType() == PacketType.Play.Server.MAP_CHUNK) {
                        ...
                        if (metaValues.size() == 0)
                            swearBlock =
                                (pl.persistence && pl.sql.swearBlock(p.getUniqueId())) || plugin
                                    .getConfig().getBoolean("swearing.defaultStatus");
                        else {
                            boolean thisPlugin = false;
                            for (MetadataValue metadataValue : metaValues)
                                thisPlugin =
                                    thisPlugin ? thisPlugin : metadataValue.getOwningPlugin() == pl;
                            if (!thisPlugin)
                                swearBlock =
                                    (pl.persistence && pl.sql.swearBlock(p.getUniqueId())) || plugin
                                        .getConfig().getBoolean("swearing.defaultStatus");
                            ...
                            for (String key : nbt.getKeys()) {
                                if (key.contains("Text")) {
                                    if (!nbt.getString(key).equals("{\"text\":\"\"}")) {
                                        ...
                                        String text =
                                            Json.jsonToColourCode(json.replace("&", "§§"), "&0");
                                        List<String> mList =
                                            pl.swearList.getList().get("multiplier");
                                        List<String> nomList =
                                            pl.swearList.getList().get("nomultiplier");
                                        List<String> oList =
                                            pl.swearList.getList().get("onlymatch");
                                        ...
                                        for (String word : both) {
                                            StringBuilder regex = new StringBuilder(
                                                "((?<=&[a-fk-o\\d])|(^|(?<=\\s)))(")
                                                .append(word.charAt(0)).append("((&[a-fk-o\\d]))|")
                                                .append(word.charAt(0)).append(")+");
                                            for (int i = 1; i < word.length(); i++)
                                                regex.append("\\s*((").append(word.charAt(i))
                                                    .append("|&[a-fk-o\\d]))+");
                                            Matcher matcher = Pattern.compile(regex.toString())
                                                .matcher(regexedMsg);
                                            while (matcher.find() && matcher.group().length() > word
                                                .length()) {
                                                int length = matcher.group().length();
                                                if (pl.ignoreSwear.stream().anyMatch(Arrays.asList(
                                            ...
                                            if (Bukkit.getPlayer(temp) == null) {
                                                try {
                                                    ...
                                                    if (!(badmul.size() > 1 || badt.size() > 1
                                                        || bado.size() > 1 || (badmul.size() > 0 &&
                                                        StringUtils
                                                            .countMatches(testTemp, badmul.get(0))
                                                            > 1) || (badt.size() > 0 && StringUtils
                                                        .countMatches(testTemp, badt.get(0)) > 1))
                                                        || (bado.size() > 0 && StringUtils
                                                        .countMatches(testTemp, bado.get(0)) > 1)) {
                                                        bad1 = badmul.size() > 0 ?
                                                            badmul.get(0) :
                                                            null;
                                                    ...
                                                    if (multiple || (bad1 != null && !bad1
                                                        .equals("") && !bad1.isEmpty() && (
                                                        testTemp.length() <= multiplier * bad1
                                                            .length())) || (bad2 != null
                                                        && testTemp.length()
                                                        <= bad2.length() + incr) || bad3 != null) {
                                                        actuallyEdited = true;
                                                        continue;
                                                    }
                                        ...
                                        if (actuallyEdited) {//only actually resend/etc the packet if we've edited it.
                                            ...
                                            if (s3.endsWith(" "))
                                                s3 = s3.substring(0, s3.length() - 1);
                                            String message =
                                                Json.colourCodeToJson(s3 + "\"}]", "&");
                                            if (message.startsWith("\"},"))
                                                message = message.substring(3, message.length());

                                            String sign = message.replace("§", "&")
                                                .substring(0, message.length() - 1);

                                            s3 = sign;

                                            HashMap<String, Boolean> chatHasStyle = new HashMap<>();
                                            for (String style : new String[] {"bold", "italics",
                                                "underline"})
                                                chatHasStyle.put(style,
                                                    sign.contains("\"" + style + "\":true"));
                                            ...
                                            for (String part : chatParts) {
                                                ...
                                                for (String style : chatHasStyle.keySet())
                                                    if (chatHasStyle.get(style) && !partBuilder
                                                        .toString().contains("\"" + style + "\":"))
                                                        partBuilder.append(",\"").append(style)
                                                            .append("\":false");
                                            ...
                                            try {
                                                ...
                                                pl.getLogger().severe(
                                                    "Error Editing Sign Packet. Please report this to GitLab");
                                    ...
                                    for (String word : both) {
                                        ...
                                        for (int i = 1; i < word.length(); i++)
                                            regex.append("\\s*((").append(word.charAt(i))
                                                .append("|&[a-fk-o\\d]))+");
                                        Matcher matcher =
                                            Pattern.compile(regex.toString()).matcher(regexedMsg);
                                        while (matcher.find() && matcher.group().length() > word
                                            .length()) {
                                            int length = matcher.group().length();
                                            if (pl.ignoreSwear.parallelStream().anyMatch(Arrays
                                                .asList(matcher.group().toLowerCase()
                                                    .split(" "))::contains)) {
                                                regexedMsg = regexedMsg.replace(matcher.group(),
                                                    new String(new char[length])
                                                        .replace('\0', '*'));
                                    ...
                                    for (String w : words) {//iterate through all the words in the packet's message
                                        ...
                                        if (Bukkit.getPlayer(temp) == null) {
                                            try {
                                                ...
                                                List<String> badmul =
                                                    pl.swearList.getList().get("multiplier")
                                                        .parallelStream().filter(testTemp::contains)
                                                        .collect(Collectors.toList());
                                                List<String> badt =
                                                    pl.swearList.getList().get("nomultiplier")
                                                        .parallelStream().filter(testTemp::contains)
                                                        .collect(Collectors.toList());
                                                List<String> bado = oList.parallelStream()
                                                    .filter(testTemp::equalsIgnoreCase)
                                                    .collect(Collectors.toList());
                                                String bad1 = null;
                                                String bad2 = null;
                                                String bad3 = null;
                                                ...
                                                if (!(badmul.size() > 1 || badt.size() > 1
                                                    || bado.size() > 1 || (badmul.size() > 0 &&
                                                    StringUtils
                                                        .countMatches(testTemp, badmul.get(0)) > 1)
                                                    || (badt.size() > 0 &&
                                                    StringUtils.countMatches(testTemp, badt.get(0))
                                                        > 1)) || (bado.size() > 0 &&
                                                    StringUtils.countMatches(testTemp, bado.get(0))
                                                        > 1)) {
                                                ...
                                                if (multiple || (bad1 != null && !bad1.equals("")
                                                    && !bad1.isEmpty() && (testTemp.length()
                                                    <= multiplier * bad1.length())) || (bad2 != null
                                                    && testTemp.length() <= bad2.length() + incr)
                                                    || bad3 != null) {
                                                    c.append(w.replaceAll(
                                                        "(((?<!&)[a-fk-o\\d])|[g-jp-zA-Z_])", "*"))
                                                        .append(" ");
                                                    actuallyEdited = true;
                                                    continue;
